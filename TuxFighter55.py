...
TuxFighterVersion = 55
print("please start the game a second time if it does not run at the first time")
"""
 _____           _____ _       _     _            
...
if os.name == 'posix':
    ...
    try:
        ...
        print('I could not import from  .TuxFighter ... please try again, i create directories now')
...
if not pygame.image.get_extended():
    print("Sorry, extended image module required")
    raise SystemExit
...
if sys.version < "3.10":
    ...
    print("You need Version 3.8 or better. Look at www.python.org")
    xx = input("press enter")
    print("python version too low")
    raise SystemExit
...
def load_image(file, colorkey=None):
    ...
    try:
        ...
        print('Could not load image "%s" %s'%(file, pygame.get_error()))
        raise SystemExit
    if colorkey is None:
...
def startmenu(resx, resy, winstyle = 0):
    ...
    try:         # try to load and interpret ini-File:
        ...
        while True:
            ...
            if len(zeile) == 0: #End of File
                break
            string = zeile.split(':')[1]
            string = string.strip('\n')
            params[zeile.split(':')[0]] = string
        f.close()
        if "MOUSE" in params:
    ...
    if MOUSE == False and JOYSTICK == False:
        ...
        print("error: JOYSTICK and MOUSE selected.")
        raise SystemExit
    if KEYSET == 'WASD':
           ...
           # more keyboard setups ?
            ...
            while True:
                ...
                if y3 == 0: # visit homepage
                    ...
                    webbrowser.open_new("https://github.com/horstjens/tuxfighter")
                elif y3 == 1:   #email to horst
                    mailtext = "i paid nothing for this game, but i *must* tell you:"
                    webbrowser.open_new("mailto:horstjens@gmail.com?subject=" + subjecttext + "&body=" + mailtext)
...
def collision(TuxPos, TuxDreh, enemyrec):
    ...
    for controlpoint in range(1,int(360/TUX_CONTROLANGLE)+1):
...
def startgame(screen, background, SCREENRECT, MOUSE=False, JOYSTICK=False, KEYSET="WASD",CHEAT=False,
    ...
    if JoystickControl == True:
        ...
        if not pygame.joystick.get_count():
            print("Sorry, no Joystick detcted")
            raise SystemExit
            ...
            for alien in pygame.sprite.spritecollide(rakete, aliens, False):
                ...
                if rakete.askstoned() == True:
                    ...
                    if rakete.askbounce() > 0:
                        if rakete.askbounce() in HITMATRIX:
